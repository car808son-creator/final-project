...
let secondCounter = 0; // x 2 per click
...
miniImgs.forEach((miniImg, index) => {
  ...
  miniImg.addEventListener('click', function() {
    ...
    if (miniImg.style.bottom !== '10px' && miniImg.style.filter !== 'grayscale(100%)') {
      ...
      const reward = miniImg.dataset.reward;
      rice += parseInt(reward);
      updateScoreDisplay();
      checkUpgradeButtons();
...
function checkUpgradeButtons() {
  ...
  if (button2Unlocked) {
    ...
    if (rice >= 750) {
      upgradeButton2.className = 'enabled';
    } else {
      upgradeButton2.className = 'disabled';
    }
  }
  ...
  if (button3Unlocked) {
    ...
    if (rice >= 1500) {
      upgradeButton3.className = 'enabled';
    } else {
      upgradeButton3.className = 'disabled';
    }
  }
  ...
  if (button4Unlocked) {
    ...
    if (rice >= 5000) {
      upgradeButton4.className = 'enabled';
    } else {
      upgradeButton4.className = 'disabled';
    }
  }
  ...
  if (button5Unlocked) {
    ...
    if (rice >= 10000) {
      upgradeButton5.className = 'enabled';
    } else {
      upgradeButton5.className = 'disabled';
    }
  }
}
